...
Qin=Qstored+Qout 
Qout=  PR , C=dVdP → dVdt=CdPdt=Qstored
Qin=CdPdt+PR
Governing equation: CdP(t)dt=Qin(t)+P(t)R
In order to find the transfer function for the system, the following steps can be done:
Apply laplace transforms: CsP(s)=Qin(s)-1RP(s) → (Cs+1R)P(s)=Qin(s)
End up with equation for flowrate to pressure: GP/Q(s)=P(s)Qin(s)=1Cs + 1R
Applying a time constant of =RC, then equation then becomes GP/Q(s)=RRCs+1=Rs+1
One thing to notice with these equations is that due to the nature of how a heart works and pumps blood, the flow in will not always be steady. In fact, when the heart is in diastole the flow in is actually 0, meaning Qin=0 at those times. This changes the following equations so dPdt=-PRC and therefore P(t)=P0e-tRC The combination of these equations show how the windkessel effect ensures smooth blood flow from the heart to the body through compliance and resistance. 
The system can be modeled using a sinusoidal input to take into account the diastole and systole parts of a heart’s beating. The heart beat is Qin(t) which changes, and can be modeled using Qin(t)=Q0+Asin(t) as a sinusoidal input, resulting in the graph shown below. 
...
Since we still want our heart to have a sinusoidal beat, our reference input will be:
Qref= Q0+Asin(wt)
Our error is then just the difference between the output and the reference:
e(t) = Qref(t) - Qout(t)
 We also have a controller that is based off of the error:
Qin(t) = Ke(t)
When you then add this with the Windkessel dynamics:
dPdt= QinC-PRC
Qout(t)= P(t)R
dPdt= KCQref(t)-PRC(k+1)
From these equations, we can find the transfer function for the closed loop system, which can be useful in evaluating if there are any poles. The laplace transform, if we assume initial conditions of zero, is: 
sP(s) = KCQref(t)-K+1RCP(s)
Now we can get G:
Gp= P(s)Qref(s)= KCs + K + 1RC
GQ=GPR=KRCs + K + 1RC

From this, we can see that the pole of the function is
 -K+1RC
...
Using the closed loop model: 
dPdt=KCQref-PRC(K+1)
 With Qref =Q0+Asin(2t)
dPdt+P(t)RC(K+1)=KCQref(t)
Therefore the closed loop time constant is: CL=K+1RC
This shows that as K increases, the closed loop time constant decreases and there is faster disturbance rejection. Also, the derivation of the time constant aligns with the concept that “the two-element Windkessel predicts that in diastole, when the aortic valve is closed, pressure will decay exponentially with a characteristic decay time RC” [2].
The disturbance transfer function helps the interpretation of the effect of disturbance rejection on arterial pressure and arterial outflow The outflow with an added disturbance d(t) can be modeled as Qout(t) = P(t)R+d(t)
Using Windkessel dynamics: dPdt=QinC-PRC
Using Laplace transform to find pressure transfer function (P(s)D(s)): 
Qin​(s):​=K(Q)ref​(s)−Qout​(s))=K(Qref(s)−P(s)R​−D(s))
CsP(s) = Qin(s)-P(s)R=KQref(s)-KRP(s)-KD(s)-1RP(s) 
(Cs+K+1R)P(s)=KQref(s)-KD(s)
P(s) = KCs+K+1RQref(s)-KCs+K+1RD(s)
Gpressure(s)= P(s)D(s)= -KCs+(K+1)R
Using Laplace transform to find output flow transfer function (Qout(s)D(s)):
Qout(t) = P(t)R+d(t)
Qout(s) = P(s)R+D(s)
Plug in pressure transfer function:
Qout(s) = 1R(-KCs+(K+1)RD(s))+D(s)= (1-KR(Cs +(K+1)R))D(s)= (1-KCRs+K+1)D(s)
Gflow(s)=Qout(s)D(s)=1-KCRs+K+1=CRs+1CRs+K+1
From the transfer functions describing the output pressure and output flow from an input disturbance, long lasting disturbances like step inputs and shifts in posture correlate to when s goes to zero and the system responds by 1K+1. This shows that increasing K improves the disturbances rejection for low frequency disturbances
